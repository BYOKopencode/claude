...
class UserConfig(BaseModel):
    ...
    client_cookie: str = Field(min_length=1)
    client_uat: str = Field(min_length=1)
    cf_bm: str = Field(min_length=1)
    cfuvid: str = Field(min_length=1)
...
class Settings(BaseSettings):
    ...
    def load_users(self) -> list[UserConfig]:
        ...
        if not merged:
            raise RuntimeError(
                "No OmniRogue users configured. Add users.json, OMNIROGUE_USERS, "
